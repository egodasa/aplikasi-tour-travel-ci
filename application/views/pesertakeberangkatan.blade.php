...
@section('content')
  <a href="{{ site_url('jadwalkeberangkatan') }}" class="btn btn-primary">Kembali</a>
  <h4>Detail Jadwal</h4>
  ...
  <table class="table table-bordered table-stripped">
    ...
    @foreach($data_list as $nomor => $data)
      <tr>
        ...
        <td>{{ $data['status'] }}</td>
        <td>
          <a href="{{ site_url('jadwalkeberangkatan/peserta/hapus?id_jadwal='.$data['id_jadwal'].'&id_keberangkatan='.$data['id_keberangkatan']) }}">Hapus</a>
        </td>
      </tr>
    @endforeach
  </table>
